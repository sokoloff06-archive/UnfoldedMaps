...
public class EarthquakeCityMap extends PApplet {
	...
	private void clickQuake(Marker m){
		for (Marker quake:quakeMarkers){
			if (!quake.equals(m)){
		...
		for (Marker city:cityMarkers){
			...
			if (e.threatCircle() < e.getDistanceTo(city.getLocation())){
				city.setHidden(true);
			}
		}
	}
	
	private void clickCity(Marker m){
		for (Marker city:cityMarkers){
			if(!city.equals(m)){
				city.setHidden(true);
			}
		...
		for (Marker quake:quakeMarkers){
			...
			if (q.getDistanceTo(m.getLocation()) > q.threatCircle()){
				quake.setHidden(true);
			}
		}
	}
