...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {
		...
		fill(color(255, 0, 0));
		triangle(50, 120, 55, 130, 45, 130);
		noFill();
		ellipse(50, 152, 10, 10);
		rect(45, 172, 10, 10);
	...
	private void printQuakes() {
		...
		for (Marker c : countryMarkers) {
			...
			for (Marker q : quakeMarkers) {
				if (q.getProperty("country") == c.getProperty("name")) {
					counter++;
				}
			}
			if (counter > 0) {
		...
		counter = 0;
		for (Marker q : quakeMarkers) {
			if (q instanceof OceanQuakeMarker) {
				counter++;
			}
		}
